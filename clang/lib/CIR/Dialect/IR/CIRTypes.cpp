...
// Spells the address space of `!cir.ptr` and `cir.global` as either
// `lang_address_space(x)` or `target_address_space(n)`.
...
// Custom parser/printer for the optional `inheritance` parameter in
// `!cir.data_member` and `!cir.method`.
mlir::ParseResult parseMSInheritance(mlir::AsmParser &p,
                                     cir::MSInheritanceModelAttr &attr);

void printMSInheritance(mlir::AsmPrinter &printer,
                        cir::MSInheritanceModelAttr attr);
...
// The cir.ptr data-layout entry holds a #cir.ptr_spec attribute (see
...
MethodType::getTypeSizeInBits(const mlir::DataLayout &dataLayout,
                              mlir::DataLayoutEntryListRef params) const {
  if (auto msModel = getMSInheritance()) {
    auto voidPtrTy = cir::PointerType::get(cir::VoidType::get(getContext()));
    uint64_t ptrSize = dataLayout.getTypeSizeInBits(voidPtrTy).getFixedValue();
    switch (*msModel) {
    case MSInheritanceModel::Single:
      return llvm::TypeSize::getFixed(ptrSize);
    case MSInheritanceModel::Multiple:
    case MSInheritanceModel::Virtual:
      return llvm::TypeSize::getFixed(ptrSize + 64);
    case MSInheritanceModel::Unspecified:
      return llvm::TypeSize::getFixed(ptrSize + 128);
    }
  }
...
MethodType::getABIAlignment(const mlir::DataLayout &dataLayout,
  ...
  if (getMSInheritance()) {
    auto voidPtrTy = cir::PointerType::get(cir::VoidType::get(getContext()));
    return dataLayout.getTypeABIAlignment(voidPtrTy);
  }
...
DataMemberType::getTypeSizeInBits(const ::mlir::DataLayout &dataLayout,
  ...
  if (auto msModel = getMSInheritance()) {
    ...
    case MSInheritanceModel::Unspecified:
      return llvm::TypeSize::getFixed(96);
    }
  }
...
VPtrType::getTypeSizeInBits(const mlir::DataLayout &dataLayout,
                            mlir::DataLayoutEntryListRef params) const {
  // A vtable pointer is an ordinary data pointer; size it as a cir.ptr.
  return dataLayout.getTypeSizeInBits(
...
void printMemorySpace(mlir::AsmPrinter &p,
                      mlir::ptr::MemorySpaceAttrInterface attr) {
  if (!attr)
    return;
...
parseGlobalMemorySpace(mlir::AsmParser &p,
  ...
  if (parseMemorySpace(p, attr).failed())
    return p.emitError(loc, "failed to parse Address Space Value for GlobalOp");
  return mlir::success();
}

void printGlobalMemorySpace(mlir::AsmPrinter &printer, cir::GlobalOp,
                            mlir::ptr::MemorySpaceAttrInterface attr) {
  printMemorySpace(printer, attr);
}
...
mlir::ParseResult parseMSInheritance(mlir::AsmParser &p,
  ...
  if (p.parseKeyword(&keyword))
    return mlir::failure();
  auto model = cir::symbolizeMSInheritanceModel(keyword);
  if (!model)
    return p.emitError(loc, "expected 'single', 'multiple', 'virtual', or 'unspecified'");
  attr = cir::MSInheritanceModelAttr::get(p.getContext(), *model);
  return mlir::success();
}

void printMSInheritance(mlir::AsmPrinter &p,
                        cir::MSInheritanceModelAttr attr) {
  if (attr)
    p << cir::stringifyMSInheritanceModel(attr.getValue());
}
